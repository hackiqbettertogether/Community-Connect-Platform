...
export class UserService {
  ...
  getConnections(userId: string): Observable<Array<UserMinified>> {
    ...
    return this.http.get<any>(this.link + '/get/connections', { headers });
  }

  getFeeds(id: string): Observable<Array<Feed>> {
    const headers = { id };
    return this.http.get<any>(this.link + '/get/feeds', { headers });
  }

  postFeed(body: any): Observable<any> {
    return this.http.post<any>(this.link + '/post/feed', body);
  }

  putLike(body: any): Observable<any> {
    return this.http.put<any>(this.link + '/put/like', body);
  }

  putComment(body: any): Observable<any> {
    return this.http.put<any>(this.link + '/put/comment', body);
  }

  postQuoteOrRepost(body: any): Observable<any> {
    return this.http.post<any>(this.link + '/post/repost', body);
  }

  getDetailsForAFeed(userId: string, feedId: string): Observable<Feed> {
    const headers = { userId };
    return this.http.get<any>(this.link + '/get/feeds/' + feedId, { headers });
  }

  imageUpload(imageForm: FormData): Observable<any> {
    return this.http.post<any>(this.link + '/post/v1/upload', imageForm);
  }
  ...
  markNotificationAsRead(userId: string, notifId: string): Observable<Array<Notif>> {
    ...
    return this.http.put<any>(this.link + '/put/mark-one-notif-as-read', null, { headers });
  }

  markAllNotificationAsRead(userId: string): Observable<Array<Notif>> {
    const headers = { userId };
    return this.http.put<any>(this.link + '/put/mark-all-notifs-as-read', null, { headers });
  }

  getUserProfile(username: string, userId): Observable<Array<Feed>> {
    const headers = { userId };
    return this.http.get<Array<Feed>>(this.link + '/get/user/' + username, { headers });
  }
